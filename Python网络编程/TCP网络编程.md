...
1.  实例化(创建)TCP的 特殊套接字-->欢迎之门,通过第二个参数`SOCK_STREAM`指定. 
2.  使用`bind()`方法为服务器指定端口(标识套接字).
3.  欢迎之门--使用 `listen()`方法等待某个客户敲门
4.  使用`accept()`方法,当客户敲门时,创建一个新的`connection`套接字,用于专门和这个客户连接.
6.  使用`recv()方法`接收信息
5.  完成工作后,关闭`connection`连接.但欢迎之门的套接字保持打开,响应后续客户的敲门.
