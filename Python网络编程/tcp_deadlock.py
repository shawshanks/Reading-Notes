...
def server(host, port):
    ...
    while True:
        ...
        # accept方法返回一个元组(conn,address)
        # conn 是一个新的套接字对象,可以用来在连接上发送和接受数据
        # address是链接在另一端的被绑定的套接字的(IP)地址
...
def client(host, port, bytecount):
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    bytecount = (bytecount + 15) //16 * 16  # 舍入到16的倍数
    message = b'capitalize this!'  # 16字节的信息被重复传送
